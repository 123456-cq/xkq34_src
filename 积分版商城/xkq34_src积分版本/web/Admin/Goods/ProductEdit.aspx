    ...
    <form id="Form1" runat="server">
                ...
                <div role="tabpanel" class="tab-pane <%if (isnext != 1)
                    ...
                    <div class="exitshopinfo">
                        <div class="form-horizontal">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-xs-4">
                                    <div class="exitpa">
                                        ...
                                        <asp:HyperLink runat="server" ID="lnkEditCategory" CssClass="a" Text="[
                            ...
                            <div class="form-group">
                                <label class="col-xs-2 control-label">零元购设置：</label>
                                <div class="col-xs-4">
                                    <div class="switch" id="Div3">
                                        <asp:CheckBox ID="cbZeroByOperation" runat="server" Checked="True" />
                                    </div>
                                </div>
                            </div>

                           <div class="form-group">
                                <label class="col-xs-2 control-label">送积分设置：</label>
                                <div class="col-xs-4">
                                    <div class="switch" id="Div1">
                                        <asp:CheckBox ID="cbIntegralOperation" runat="server" Checked="True"/>
                                    </div>
                                </div>
                            </div>

                            <div class="form-group">
                                <label for="shopname" class="col-xs-2 control-label">送积分：</label>
                                <div class="col-xs-4">
                                <asp:TextBox runat="server" class="form-control" ID="txtIntegral" MaxLength="5" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" Enabled="false">
                                </asp:TextBox>
                                <%--<Hi:TrimTextBox runat="server" CssClass="form-control" ID="txtIntegral" MaxLength="5" />--%>
                                </div>
                            </div>

                            <div class="form-group">
                                <label class="col-xs-2 control-label">积分兑换设置：</label>
                                <div class="col-xs-4">
                                    <div class="switch" id="Div2">
                                        <asp:CheckBox ID="cbPointsSetUp" runat="server" Checked="true" />
                                    </div>
                                </div>
                            </div>

                            <div class="form-group">
                                <label for="shopname" class="col-xs-2 control-label">积分兑换上限：</label>
                                <div class="col-xs-4">
                                <asp:TextBox runat="server" class="form-control" ID="txtIntegralCeiling" MaxLength="5" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"  Enabled="false"></asp:TextBox>
                                <%--<Hi:TrimTextBox runat="server" CssClass="form-control" ID="txtIntegralCeiling" MaxLength="5"/>--%>
                                </div>
                            </div>
                    ...
                    <div class="exitshopinfo resize bg">
                        <div class="form-horizontal">
                            <h3>分销佣金设置</h3>
                            <div class="form-group">
                                <label class="col-xs-2 control-label">使用分销佣金设置：</label>
                                <div class="col-xs-4">
                        ...
                        <div class="form-horizontal">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-xs-4">
                                    <div class="setradio">
                                        <label>
                                            <asp:RadioButton runat="server" ID="radOnSales" GroupName="SaleStatus"></asp:RadioButton>立即上架</label>
                                    </div>
                                    <div class="setradio">
                                        <label>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                 ...
                 $('#Div3').on('switch-change', function (e, data) {//点击零元购设置
                    if (data.value) {
                    }else {
                       $("#Div2 div").attr("class",'switch-on switch-animate');//积分兑换设置打开
                       $("*[id$=txtIntegralCeiling]").attr("disabled",true);//禁用积分兑换设置文本
                       $("*[id$=txtIntegralCeiling]").val('');
                       $("#Div1 div").attr("class",'switch-on switch-animate');//送积分设置打开
                       $("*[id$=txtIntegral]").attr("disabled",true);//禁用送积分设置文本
                       $("*[id$=txtIntegral]").val('');
                    }
                })

                $('#Div1').on('switch-change', function (e, data) {//点击送积分设置
                    if (data.value) {
                       $("*[id$=txtIntegral]").val('');
                       $("*[id$=txtIntegral]").attr("disabled",true);//禁用送积分文本
                    }else {
                       $("*[id$=txtIntegral]").attr("disabled",false);//取消禁用送积分文本
                       $("#Div2 div").attr("class",'switch-on switch-animate');//积分兑换设置打开
                       $("*[id$=txtIntegralCeiling]").attr("disabled",true);//禁用积分兑换设置文本
                       $("*[id$=txtIntegralCeiling]").val('');
                        $("#Div3 div").attr("class",'switch-on switch-animate');//零元购设置打开
                    }
                })

               $('#Div2').on('switch-change', function (e, data) {//点击积分兑换设置
                    if (data.value) {
                      $("*[id$=txtIntegralCeiling]").val('');
                      $("*[id$=txtIntegralCeiling]").attr("disabled",true);//禁用积分兑换文本
                    }else {
                      $("*[id$=txtIntegralCeiling]").attr("disabled",false);//取消禁用积分兑换文本
                      $("#Div1 div").attr("class",'switch-on switch-animate');//送积分设置打开
                      $("*[id$=txtIntegral]").attr("disabled",true);//禁用送积分设置文本
                      $("*[id$=txtIntegral]").val('');
                      $("#Div3 div").attr("class",'switch-on switch-animate');//零元购设置打开
                ...
                <%}%>
                    ...
                    $('#aspnetForm').formvalidation({
                     'ctl00$ContentPlaceHolder1$txtIntegral':{
                        validators: {
                            regexp: {
                                regexp: /^[1-9]\d*$/,
                                message: '送积分只能输入整数型数值'
                                  }
                            }
                    },
                    })
                    $('#aspnetForm').formvalidation({
                    'ctl00$ContentPlaceHolder1$txtIntegralCeiling': {
                        validators: {
                            regexp: {
                                regexp: /^[1-9]\d*$/,
                                message:'积分兑换上限只能输入整数型数值'
                            }
                        }
                    }
                  })
