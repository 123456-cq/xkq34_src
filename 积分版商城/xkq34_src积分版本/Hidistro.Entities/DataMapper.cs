namespace Hidistro.Entities
{
    ...
    public static class DataMapper
    {
        ...
        public static ProductInfo PopulateProduct(DataRow reader)
        {
            ...
            if (DBNull.Value != reader["integral"])
            {
                info.integral = reader["integral"].ToString();
            }
            if (DBNull.Value != reader["IntegralCeiling"])
            {
                info.IntegralCeiling = reader["IntegralCeiling"].ToString();
            }
            if (DBNull.Value != reader["FristPrice"])
            {
                info.FristPrice = Convert.ToInt32(reader["FristPrice"]);
            }
