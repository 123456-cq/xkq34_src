...
namespace HiTemplate
{
    ...
    public class GoodsMobule : RazorModuleWebControl
    {
        ...
        public string GetDataJson()
        {
            ...
            //获得此cookie对象
            if (HttpContext.Current.Request.Cookies["goodsMobule"] != null)
            {
                string a = HttpContext.Current.Request.Cookies["goodsMobule"].Value[1].ToString();
                var jobj = JsonConvert.DeserializeObject<JObject>(a);
                string value = jobj["DataUrl"].ToString();
            }
            string jsonstr = "";
            try
            {
                ...
                //参数
                string query = string.Format("ShowPrice={0}&Layout={1}&ShowIco={2}&ShowName={3}&IDs={4}&ShopNumber={5}", base.ShowPrice, base.Layout, base.ShowIco, base.ShowName, this.IDs, this.ShopNumber);
                ...
                StreamReader reader = new StreamReader(response.GetResponseStream(), Encoding.UTF8);//获取商品数据
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            try
            {
                ...
                String jsonData = this.GetDataJson();//获取所有数据
