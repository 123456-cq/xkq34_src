namespace Hidistro.SaleSystem.Vshop
{
    ...
    public static class MemberProcessor
    {
        ...
        {
            bool flag = false;
            if (order.OrderStatus == OrderStatus.WaitBuyerPay)//等待卖家付款
            {
            ...
            else if (order.OrderStatus == OrderStatus.Closed)//已关闭订单
            {

                if (order.CheckAction(OrderActions.SELLER_CLOSE))
                {
                    flag = new OrderDao().DeleteOrder(order);
                }

            }
            //else if (order.OrderStatus == OrderStatus.Finished)//已完成订单
            //{
        ...
        public static MemberInfo GetCurrentMember()//获取剩余积分
        {
        ...
        public static decimal GetIntegral(int userId)//获取总积分
        {
