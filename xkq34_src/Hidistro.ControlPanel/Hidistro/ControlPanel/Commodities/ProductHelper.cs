namespace Hidistro.ControlPanel.Commodities
{
    ...
    public static class ProductHelper
    {
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        public static DbQueryResult GetSendsIntegralProducts(ProductQuery query)
        {
            return new ProductDao().GetSendsIntegralProducts(query);
        }
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        public static DbQueryResult GetPointsForProducts(ProductQuery query)
        {
            return new ProductDao().GetPointsForProducts(query);
        }
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        public static DbQueryResult GetZeroByProducts(ProductQuery query)
        {
            return new ProductDao().GetZeroByProducts(query);
        }
        ...
        /// <returns></returns>
        public static int DelZeroByProduct(string productIds)
        {
            int count = new AdminService().DelPointsForProduct(productIds);
            if (count > 0)
            {
                EventLogs.WriteOperationLog(Privilege.OffShelfProducts, string.Format(CultureInfo.InvariantCulture, "删除成功！", new object[] { count }));
            }
            return count;
        }
        #endregion
        ...
        /// <param name="productIds"></param>
        /// <returns></returns>
        public static int DelZeroByProducts(string productIds)
        {
            int count = new AdminService().DelPointsForProduct(productIds);
            if (count > 0)
            {
                EventLogs.WriteOperationLog(Privilege.OffShelfProducts, string.Format(CultureInfo.InvariantCulture, "零元购列表批量删除了 “{0}” 件商品", new object[] { count }));
            }
            return count;
        }
        #endregion
        ...
        /// <param name="productIds"></param>
        /// <returns></returns>
        public static int DelPointsForProduct(string productIds)
        {
            int count = new AdminService().DelPointsForProduct(productIds);
            if (count > 0)
            {
        ...
        /// <param name="productIds"></param>
        /// <returns></returns>
        public static int DelPointsForProducts(string productIds)
        {
            int count = new AdminService().DelPointsForProduct(productIds);
            if (count > 0)
            {
                EventLogs.WriteOperationLog(Privilege.OffShelfProducts, string.Format(CultureInfo.InvariantCulture, "积分兑换列表批量删除了 “{0}” 件商品", new object[] { count }));
            }
            return count;
        }
        #endregion

        #region 购物送积分列表批量删除商品
        /// <summary>
        /// 购物送积分列表批量删除商品
        /// </summary>
        /// <param name="productIds"></param>
        /// <returns></returns>
        public static int DelSendsIntegralProducts(string productIds)
        {
            int count = new AdminService().DelSendsIntegralProduct(productIds);
            if (count > 0)
            {
                EventLogs.WriteOperationLog(Privilege.OffShelfProducts, string.Format(CultureInfo.InvariantCulture, "购物送积分列表批量删除了 “{0}” 件商品", new object[] { count }));
            }
            return count;
        }
        #endregion

        #region 购物送积分列表删除单个商品
        /// <summary>
        /// 购物送积分列表删除单个商品
        /// </summary>
        /// <param name="productIds"></param>
        /// <returns></returns>
        public static int DelSendsIntegralProduct(string productIds)
        {
            int count = new AdminService().DelSendsIntegralProduct(productIds);
            if (count > 0)
            {
