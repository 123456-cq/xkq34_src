    ...
    public class VshopProcess : IHttpHandler, IRequiresSessionState
    {
        ...
        Label_0BC9:
            ...
            try
            {
                ...
                if (ShoppingProcessor.CreatOrder(order))
                {
                    ...
                    if (summittype)
                    {
                        ShoppingCartProcessor.ClearShoppingCart();//清空购物车
                        ShoppingCartProcessor.ClearPoints();//清空积分兑换内容
                    }
                    try
                    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            switch (context.Request["action"])
            {
                ...
                case "Submmitorder":
                    this.ProcessSubmmitorder(context);//提交订单方法
                    return;

                case "CancelOrder":
                    this.CancelOrder(context);//取消订单
                    return;
