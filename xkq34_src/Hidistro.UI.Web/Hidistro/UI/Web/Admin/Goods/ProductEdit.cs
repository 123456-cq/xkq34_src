...
using Hidistro.UI.Web.Admin.Helper;
...
namespace Hidistro.UI.Web.Admin.Goods
{
    ...
    public class ProductEdit : ProductBasePage
    {
        ...
        protected TextBox txtIntegral;//送积分框

        protected TextBox txtIntegralCeiling;//积分兑换上限

        protected System.Web.UI.WebControls.CheckBox cbIntegralOperation;//送积分设置

        protected System.Web.UI.WebControls.CheckBox cbPointsSetUp;//积分兑换设置
        ...
        protected void Page_Load(object sender, System.EventArgs e)//加载事件
        {
            ...
            if (!this.Page.IsPostBack)
            {
                ...
                if (this.productId > 0)
                {
                    ...
                    int integrals = 0; string integralToNow = ""; int integralCeiling = 0;//送积分,积分兑换,积分兑换上限

                    #region 根据商品编号查询商品积分,积分是否抵现,积分兑换上限
                    SqlDataReader read = null;
                    try
                    {
                        string sql = "select [integral],[IntegralToNow],[IntegralCeiling] from [dbo].[Hishop_Products] where [ProductId]=@ProductId";
        ...
        private void LoadProduct(ProductInfo product, System.Collections.Generic.Dictionary<int, System.Collections.Generic.IList<int>> attrs)//商品详情控件赋值
        {
            if (product.Integral != "" & product.Integral != "0")//如果积分兑换有值则关闭积分设置
            {
                this.cbIntegralOperation.Checked = false;
                this.txtIntegral.Enabled = true;
                this.txtIntegral.Text = product.Integral.ToString();//为送积分框赋值
            }
            else
            {
            ...
            if (product.IntegralToNow == 1)//是否能积分兑换（1能,0否）
            {
                this.cbPointsSetUp.Checked = false;
                this.txtIntegralCeiling.Enabled = true;
                this.txtIntegralCeiling.Text = product.IntegralCeiling.ToString();//为积分兑换上限框赋值
            }
            else if (product.IntegralToNow == 0)
            {
                this.cbPointsSetUp.Checked = true;
                this.txtIntegralCeiling.Enabled = false;
                this.txtIntegralCeiling.Text = "";
            }
            else
            {
                this.cbPointsSetUp.Checked = true;
