...
namespace Hidistro.UI.SaleSystem.CodeBehind
{
    ...
    public class VSubmmitOrder : VMemberTemplatedWebControl
    {
        ...
        private Button btnSureIntegral;
        decimal PointFor = 0;
        ...
        protected override void AttachChildControls()
        {
            ...
            this.rptCartProducts.ItemDataBound += new RepeaterItemEventHandler(this.rptCartProducts_ItemDataBound);//绑定订单金额计算详情
            ...
            //this.btnSureIntegral = (Button)this.FindControl("btnSureIntegral");


            IList<ShippingAddressInfo> shippingAddresses = MemberProcessor.GetShippingAddresses();
            //btnSureIntegral.Attributes.Add("onclick", "return check();");
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            {
                this.SkinName = "Skin-VSubmmitOrder.html";//确认订单页面
            }
            base.OnInit(e);
        }

        private void rptCartProducts_ItemDataBound(object sender, RepeaterItemEventArgs e)//订单金额计算详情
        {
            if ((e.Item.ItemType == ListItemType.Item) || (e.Item.ItemType == ListItemType.AlternatingItem))
            {
                ...
                Literal literal2 = (Literal)e.Item.Controls[0].FindControl("litExemption");//优惠减免
                Literal literal3 = (Literal)e.Item.Controls[0].FindControl("litoldExemption");//优惠减免
                Literal literal4 = (Literal)e.Item.Controls[0].FindControl("litoldTotal");//应付总额！！
                Literal literal5 = (Literal)e.Item.Controls[0].FindControl("litTotal");//应付总额
                Literal literal6 = (Literal)e.Item.Controls[0].FindControl("litbFreeShipping");//是否包邮
                Literal literal7 = (Literal)e.Item.Controls[0].FindControl("litPointsFor");//积分兑换

                if (!String.IsNullOrEmpty(this.Page.Request.QueryString["ShopType"]))//点击立即购买进来的
                {

                }
                else
                {
                    DataTable dt = service.SelIntegralSetInShopCar(currentMember.UserId);//获取购物车商品所有商品兑换总积分
                    foreach (DataRow item in dt.Rows)
                    {
                        if (!String.IsNullOrEmpty(item["IntegralSet"].ToString()))
                        {
                            PointFor += Convert.ToDecimal(item["IntegralSet"]);//累加商品兑换积分
                        }
                    }
                    literal7.Text = PointFor.ToString();//积分兑换赋值
                }
                ...
                literal4.Text = (num6 - num5 - PointFor).ToString("F2");
            }
        }
    }
}
